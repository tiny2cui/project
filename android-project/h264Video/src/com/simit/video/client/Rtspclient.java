...
import com.simit.video.rtspclient.transport.PlainUDP;
...
//	private Context context;
	...
	public  void init(SendAndReceive send,ClientResponse response) throws Exception
	{
		client = new RTSPClient();
		client.setTransport(new PlainUDP(send));
	...
	public void handMessage(byte[] data,int type){
		if(type==FrameType.MESSAGE_VIDEO_CONTROL_S2C){
			if(client!=null){
				client.getTransport().receive(data);
			}
			
		}else if(type==FrameType.MESSAGE_VIDEO_DATA){
			if(mLanVideoPlay!=null){
				mLanVideoPlay.receive(data);
			}
	...
	public void do_option(String uri)
	{
		request_uri=uri ; 
		try {
			client.options(request_uri, null);
	...
	public void responseDESCRIBE(){
		System.out.println(resourceList);

		try {
			if(resourceList.size() > 0)
				client.setup(new URI(controlURI), nextPort(), resourceList.remove(0));
			else
...
//					String temp1 = "server_port=";
//					String sport1 = sport.substring(sport.indexOf(temp1));
//					sport1 = sport1.substring(temp1.length(), sport1.indexOf("-"));
//					server_port = Integer.parseInt(sport1);
//					
//					String temp = "client_port=";
//					sport1 = sport.substring(sport.indexOf(temp));
//					sport1 = sport1.substring(temp.length(), sport1.indexOf("-"));
//					port = Integer.parseInt(sport1);
...
//				clientResponse.response(request.getMethod());
			} 
			else
				client.teardown();
		} catch(Throwable t)
...
//			mLanVideoPlay.startThread();
	...
	private void parseParameterSet(String SDP)
	{
		...
		mLanVideoPlay = new LanVideoPlay(null, width, height,str_SPS,str_PPS);
		mLanVideoPlay.startThread();
	}
